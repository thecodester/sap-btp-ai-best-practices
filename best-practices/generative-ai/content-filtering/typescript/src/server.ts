...
import { chatWithSupport, generateParaphrase } from "./services/aiOrchestration";
...
app.post("/chatWithSupport", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    try {
      const response = await chatWithSupport(input, filterInput);
...
app.post("/generateParaphrase", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    try {
      ...
      logger.info(`Result from output content filter: ${response.data.module_results.output_filtering!.message}`);
      logger.info(
      ...
      res.status(400).json({
        ...
        llm_response: response.data.module_results.llm?.choices[0].message.content,
      });
    }
  } catch (error) {
    logger.error(error);
